...
from ._preprocessing import get_pca, select_informative_pcs, project_pca
...
class ScPredModel:
    ...
    # In scpred_py_simple/_core.py, inside the ScPredModel class

    def train(self, ref_adata, cell_type_key, kernel='linear', n_components=30):
        """
        Trains the scPred model, including feature selection of informative PCs.
        """
        ...
        # Get all principal components first
        ...
        # --- NEW: Use the function to select only informative PCs ---
        self.informative_pcs_ = select_informative_pcs(X_pca, labels)
        
        # Select the corresponding columns from the PCA matrix
        X_pca_selected = X_pca[:, self.informative_pcs_]
        
        # Train the classifier on the selected, informative features
