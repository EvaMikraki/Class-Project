...
def initial_preprocessing_steps(adata):
    ...
    Includes a safety net to prevent double log1p transformation.
    ...
    # --- Safety Net for log1p ---
    # Check if 'log1p' key exists in adata.uns, which indicates it has been log-transformed
    if 'log1p' in adata_copy.uns:
        print("  Warning: Data appears to be already log-transformed (adata.uns['log1p'] detected). Skipping log1p transformation.")
    else:
        # Log-transformation
        sc.pp.log1p(adata_copy)
        print("  Log1p transformation applied.")

    print("  Initial preprocessing (filter, normalize, log1p) finished.")
    return adata_copy
